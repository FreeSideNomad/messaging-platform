...
public class MnKafkaPublisher implements com.acme.reliable.spi.EventPublisher {
  ...
  @Override
  public void publish(
      String topic, String key, String value, java.util.Map<String, String> headers) {
    ...
    producer.send(
        ...
        (metadata, exception) -> {
          if (exception != null) {
            LOG.error(
                "Failed to publish message to topic {}, key {}: {}",
                topic,
                key,
            ...
            LOG.debug(
                "Successfully published message to topic {} partition {} offset {}",
                metadata.topic(),
                metadata.partition(),
                metadata.offset());
          }
