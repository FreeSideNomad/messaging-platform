...
public final class KafkaTopicInitializer implements ApplicationEventListener<StartupEvent> {
    ...
    public KafkaTopicInitializer(
            @io.micronaut.context.annotation.Value("${kafka.bootstrap.servers}")
            String configuredBootstrap,
            @io.micronaut.context.annotation.Value("${kafka.required-topics:events.CreateUser}")
            String requiredTopics,
            @io.micronaut.context.annotation.Value("${kafka.topic-partitions:3}") int partitions,
            @io.micronaut.context.annotation.Value("${kafka.topic-replication-factor:1}")
            int replicationFactor) {
        String envBootstrap = System.getenv("KAFKA_BOOTSTRAP_SERVERS");
        this.bootstrapServers =
                normalizeBootstrapServers(
    ...
    void ensureTopics() {
        ...
        try (Admin admin = Admin.create(props)) {
            ...
            LOG.info(
                    ...
                    missing.stream().map(NewTopic::name).collect(Collectors.toList()));
    ...
    private void waitForCluster(Admin admin)
            throws InterruptedException, ExecutionException, TimeoutException {
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                admin
                        .listTopics(new ListTopicsOptions().timeoutMs((int) RETRY_DELAY.toMillis()))
                        .names()
