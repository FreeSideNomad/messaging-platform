...
class ThroughputTest extends PerformanceTestBase {
    ...
    void testThroughput_500TPS_50Concurrent() throws Exception {
        ...
        // Assertions
        assertThat(metrics.successRate()).as("Success rate should be >99%%").isGreaterThan(0.99);

        assertThat(metrics.throughputTPS()).as("Throughput should be >400 TPS").isGreaterThan(400);

        assertThat(metrics.p95LatencyMs()).as("P95 latency should be <200ms").isLessThan(200);

        assertThat(metrics.p99LatencyMs()).as("P99 latency should be <500ms").isLessThan(500);
    }
    ...
    void testThroughput_1000TPS_100Concurrent() throws Exception {
        ...
        assertThat(metrics.successRate()).as("Success rate should be >98%%").isGreaterThan(0.98);

        assertThat(metrics.throughputTPS()).as("Throughput should be >800 TPS").isGreaterThan(800);

        assertThat(metrics.p95LatencyMs()).as("P95 latency should be <300ms").isLessThan(300);
    }
