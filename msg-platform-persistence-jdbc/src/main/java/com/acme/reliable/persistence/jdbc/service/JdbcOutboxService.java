...
/**
 * JDBC implementation of OutboxService using hybrid approach:
 * - Repository for simple operations (insert, markPublished, reschedule)
...
public class JdbcOutboxService implements OutboxService {
    ...
    public JdbcOutboxService(JdbcOutboxRepository repository, TransactionOperations<Connection> transactionOps,
    ...
    public UUID addReturningId(OutboxRow r) {
        ...
        var headersJson = r.headers() != null && !r.headers().isEmpty()
            ? Jsons.toJson(r.headers())
            : "{}";
        repository.insert(
            id,
            r.category(),
