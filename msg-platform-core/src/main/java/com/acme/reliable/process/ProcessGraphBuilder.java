...
/**
 * Fluent builder for constructing process DAGs.
 * Provides a declarative way to define process flows with branches and compensations.
 */
public class ProcessGraphBuilder {
    ...
    public class StepBuilder {
        ...
        public StepBuilder then(Class<? extends DomainCommand> nextStepClass) {
            ...
            parent.steps.put(stepName, new ProcessStep(
                stepName,
                compensationStep,
                new ProcessStep.DirectNext(nextStepName)
            ));
    ...
    public class ConditionalBuilder {
        ...
        ConditionalBuilder(ProcessGraphBuilder parent, String sourceStep,
                          String compensationStep, Predicate<Map<String, Object>> condition) {
            this.parent = parent;
        ...
        /**
         * Define the step to execute when condition is true.
         * Use this for optional branches: if(condition) { optional_step }; next_step
         */
        ...
        void completeConditional() {
            parent.steps.put(sourceStep, new ProcessStep(
                sourceStep,
                compensationStep,
    ...
    public class OptionalBranchBuilder {
         ...
         */
        public OptionalBranchBuilder withCompensation(Class<? extends DomainCommand> compensationClass) {
            this.compensationStep = ProcessSteps.stepName(compensationClass);
            return this;
        }

        /**
         * Define the continuation step after the optional branch
         * Both true and false paths will converge here
         */
        public StepBuilder then(Class<? extends DomainCommand> continuationClass) {
            ...
            conditionalBuilder.parent.steps.put(branchStepName, new ProcessStep(
                branchStepName,
                compensationStep,
                new ProcessStep.DirectNext(continuationStep)
            ));
    ...
    public class BranchBuilder {
        ...
        BranchBuilder(ConditionalBuilder conditionalBuilder, boolean isTrueBranch, String branchStepName) {
            this.conditionalBuilder = conditionalBuilder;
    ...
    public class SimpleBranchBuilder {
        ...
        SimpleBranchBuilder(ConditionalBuilder conditionalBuilder, String branchStepName, String convergenceStep) {
            this.conditionalBuilder = conditionalBuilder;
            this.branchStepName = branchStepName;
            this.convergenceStep = convergenceStep;
        }
        ...
        public StepBuilder then(Class<? extends DomainCommand> nextStepClass) {
            ...
            conditionalBuilder.parent.steps.put(branchStepName, new ProcessStep(
                branchStepName,
                compensationStep,
                new ProcessStep.DirectNext(convergenceStep)
            ));
    ...
    public class ParallelBranchBuilder {
        ...
        ParallelBranchBuilder(ProcessGraphBuilder parent, String sourceStep, String sourceCompensationStep) {
            this.parent = parent;
            this.sourceStep = sourceStep;
        ...
        public StepBuilder joinAt(Class<? extends DomainCommand> joinStepClass) {
            ...
            java.util.List<String> branchNames = branches.stream()
                .map(b -> b.branchStepName)
                .toList();
            ...
            for (ParallelBranch branch : branches) {
                parent.steps.put(branch.branchStepName, new ProcessStep(
    ...
    public class ParallelBranchConfig {
        ...
        ParallelBranchConfig(ParallelBranchBuilder parentBuilder, ParallelBranchBuilder.ParallelBranch branch) {
