...
/**
 * Abstract base class for Process Manager orchestration engine.
 * Contains all the domain logic for coordinating multi-step business processes.
 * Subclasses provide infrastructure concerns (transactions, DI, lifecycle).
 */
public abstract class BaseProcessManager {
     ...
     */
    public UUID startProcess(String processType, String businessKey, Map<String, Object> initialData) {
        ...
        executeInTransaction(() -> {
            ...
            LOG.info("Started process: {} type={} key={} step={}",
                processId, processType, businessKey, initialStep);
            ...
            try {
                ...
                ProcessEvent failEvent = new ProcessEvent.ProcessFailed("Failed to start: " + e.getMessage());
    ...
    public void handleReply(UUID correlationId, UUID commandId, CommandReply reply) {
        executeInTransaction(() -> {
            ...
            LOG.debug("Handling reply for process: {} command={} status={}",
                instance.processId(), commandId, reply.status());
    ...
    /**
     * Execute a process step by sending a command
    ...
    private void executeSequentialStep(ProcessInstance instance, String step) {
        ...
        UUID commandId = getCommandBus().accept(
            step,
            idempotencyKey,
        ...
        LOG.info("Executed step: process={} step={} command={}",
            instance.processId(), step, commandId);
    }

    /**
     * Execute a parallel step - spawn multiple commands
     */
    private void executeParallelStep(ProcessInstance instance, ProcessConfiguration definition,
                                       ProcessGraph graph, String step) {
        ...
        if (branches.isEmpty() || joinStep == null) {
            LOG.error("Invalid parallel step configuration: step={} branches={} joinStep={}",
                step, branches.size(), joinStep);
            throw new IllegalStateException("Invalid parallel step: " + step);
        }

        LOG.info("Executing parallel step: process={} step={} branches={} joinStep={}",
            instance.processId(), step, branches.size(), joinStep);
        ...
        for (String branch : branches) {
            ...
            LOG.info("Spawned parallel branch: process={} parent={} branch={} command={}",
                instance.processId(), step, branch, commandId);
        }
    }
     ...
     */
    private void handleStepCompleted(ProcessInstance instance, ProcessConfiguration definition,
                                      UUID commandId, CommandReply reply) {
     ...
     */
    private void handleParallelBranchCompleted(ProcessInstance instance, ProcessConfiguration definition,
                                                 UUID commandId, CommandReply reply, String completedBranch) {
        ...
        boolean allComplete = parallelState.values().stream()
            .allMatch(status -> "COMPLETED".equals(status));

        if (!allComplete) {
            // Wait for other branches - just update state
            ProcessInstance updated = instance.update(ProcessStatus.RUNNING, instance.currentStep(), newData, 0);
            getProcessRepository().update(updated, event);

            long completedCount = parallelState.values().stream()
                .filter(status -> "COMPLETED".equals(status))
                .count();

            LOG.info("Parallel branch completed, waiting for others: process={} branch={} completed={}/{}",
                instance.processId(), completedBranch, completedCount, parallelState.size());
            return;
        }

        // All branches complete - proceed from join step
        LOG.info("All parallel branches completed: process={} parentStep={} branches={}",
            instance.processId(), parentStep, parallelState.size());
        ...
        Optional<String> nextStep = getGraph(instance.processType()).getNextStep(instance.currentStep(), newData);
        ...
        if (nextStep.isPresent()) {
            ...
            LOG.info("Moving to next step after parallel join: process={} from={} to={}",
                instance.processId(), instance.currentStep(), nextStep.get());
            ...
            // Process complete
            ProcessInstance completed = updated.update(ProcessStatus.SUCCEEDED, instance.currentStep(), newData, 0);
            ...
            LOG.info("Process completed successfully after parallel execution: {} type={} key={}",
                instance.processId(), instance.processType(), instance.businessKey());
        }
    }
     ...
     */
    private void handleSequentialStepCompleted(ProcessInstance instance, ProcessConfiguration definition,
                                                 UUID commandId, CommandReply reply) {
        // Merge reply data into process data
        Map<String, Object> newData = new HashMap<>(instance.data());
        newData.putAll(reply.data());
        ...
        if (nextStep.isPresent()) {
            ...
            ProcessInstance updated = instance
                .update(ProcessStatus.RUNNING, nextStep.get(), newData, 0);

            getProcessRepository().update(updated, event);

            LOG.info("Process step completed, moving to next: process={} from={} to={}",
                instance.processId(), instance.currentStep(), nextStep.get());
            ...
            // Process complete
            ProcessInstance completed = instance
                .update(ProcessStatus.SUCCEEDED, instance.currentStep(), newData, 0);

            ProcessEvent completeEvent = new ProcessEvent.ProcessCompleted("All steps completed successfully");
            getProcessRepository().update(completed, completeEvent);

            LOG.info("Process completed successfully: {} type={} key={}",
                instance.processId(), instance.processType(), instance.businessKey());
        }
    }
     ...
     * Handle step failure
     */
    private void handleStepFailed(ProcessInstance instance, ProcessConfiguration definition,
                                   UUID commandId, CommandReply reply) {
     ...
     */
    private void handleParallelBranchFailed(ProcessInstance instance, ProcessConfiguration definition,
                                             UUID commandId, CommandReply reply, String failedBranch) {
        String error = reply.error();

        LOG.error("Parallel branch failed: process={} branch={} error={}",
            instance.processId(), failedBranch, error);
        ...
        handlePermanentFailure(instance, definition, event,
            "Parallel branch failed: " + failedBranch + " - " + error);
    }

    /**
     * Handle sequential step failure
     */
    private void handleSequentialStepFailed(ProcessInstance instance, ProcessConfiguration definition,
                                             UUID commandId, CommandReply reply) {
        String error = reply.error();
        ...
        if (retryable && instance.retries() < maxRetries) {
            ...
            LOG.warn("Step failed, retrying: process={} step={} attempt={}/{}",
                instance.processId(), instance.currentStep(), newRetries, maxRetries);
            ...
            // Permanent failure
            LOG.error("Step failed permanently: process={} step={} error={}",
                instance.processId(), instance.currentStep(), error);
     ...
     * Handle step timeout
     */
    private void handleStepTimedOut(ProcessInstance instance, ProcessConfiguration definition,
                                     UUID commandId, CommandReply reply) {
     ...
     */
    private void handlePermanentFailure(ProcessInstance instance, ProcessConfiguration definition,
                                         ProcessEvent triggerEvent, String error) {
        if (getGraph(instance.processType()).requiresCompensation(instance.currentStep())) {
            LOG.info("Starting compensation for process: {} step={}",
                instance.processId(), instance.currentStep());
            ...
            LOG.error("Process failed without compensation: {} type={} error={}",
                instance.processId(), instance.processType(), error);
        }
    }
    ...
    private void startCompensation(ProcessInstance instance, ProcessConfiguration definition) {
        Optional<String> compensationStep = getGraph(instance.processType()).getCompensationStep(instance.currentStep());
        ...
        if (compensationStep.isPresent()) {
            ...
            LOG.info("Started compensation step: process={} step={} command={}",
                instance.processId(), step, commandId);
        } else {
     ...
     */
    private void handleStepExecutionFailure(ProcessInstance instance, ProcessConfiguration definition, String error) {
        ProcessEvent event = new ProcessEvent.StepFailed(
            instance.currentStep(),
            "N/A",
            error,
