...
class BaseProcessManagerTest {
  ...
  class StartProcessTests {
    ...
    void testStartProcess() {
      ...
      verify(mockCommandBus)
          .accept(
              eq("Step1"), contains(processId.toString()), eq("biz-123"), anyString(), anyMap());

      // Verify process updated to RUNNING
      verify(mockRepository)
          .update(
              argThat(instance -> instance.status() == ProcessStatus.RUNNING),
              any(ProcessEvent.class));
    }
  ...
  class ReplyHandlingTests {
    ...
    void testHandleReplyCompleted() {
      ...
      ProcessInstance instance =
          ProcessInstance.create(processId, "TestProcess", "key", "Step1", Map.of("key", "value"))
      ...
      CommandReply reply =
          CommandReply.completed(commandId, processId, Map.of("result", "success"));
      ...
      verify(mockRepository, atLeastOnce())
          .update(any(ProcessInstance.class), any(ProcessEvent.class));
    }
    ...
    void testHandleReplyCompletesProcess() {
      ...
      ProcessInstance instance =
          ProcessInstance.create(processId, "TestProcess", "key", "Step3", Map.of())
              .withStatus(ProcessStatus.RUNNING);
    ...
    void testHandleReplyFailedWithRetry() {
      ...
      ProcessInstance instance =
          ProcessInstance.create(processId, "TestProcess", "key", "Step1", Map.of())
              .withStatus(ProcessStatus.RUNNING);
      ...
      verify(mockCommandBus).accept(eq("Step1"), anyString(), anyString(), anyString(), anyMap());
    }
  ...
  class CompensationTests {
    ...
    void testCompensationOnFailure() {
      ...
      // Verify compensation started - update called twice (compensating status + compensation
      // event)
      verify(mockRepository, atLeast(1))
          .update(
      ...
      verify(mockCommandBus)
          .accept(
              eq("CompensateStep1"), contains("COMPENSATE"), anyString(), anyString(), anyMap());
    }
    ...
    void testNoCompensationRequired() {
      ...
      verify(mockRepository)
          .update(argThat(inst -> inst.status() == ProcessStatus.FAILED), any(ProcessEvent.class));
    }
  }
  ...
  class EdgeCasesTests {
    ...
    void testCompensationWithNoStep() {
      ...
      verify(mockRepository, atLeastOnce())
          .update(
              argThat(inst -> inst.status() == ProcessStatus.COMPENSATED), any(ProcessEvent.class));
    }
  }
  ...
  private static class TestProcessManager extends BaseProcessManager {
    ...
    TestProcessManager(
        ProcessRepository repository, CommandBus commandBus, List<Runnable> transactionCallbacks) {
  ...
  class ParallelExecutionTests {
    ...
    void testExecuteParallelStep() {
      ...
      verify(mockRepository, atLeastOnce())
          .update(argThat(inst -> inst.currentStep().equals("JoinStep")), any(ProcessEvent.class));
    }
    ...
    void testParallelBranchCompletionFirst() {
      ...
      ProcessInstance instance =
          ProcessInstance.create(processId, "ParallelProcess", "key", "JoinStep", data)
              .withStatus(ProcessStatus.RUNNING);
      ...
      verify(mockCommandBus, never())
          .accept(eq("AfterJoin"), anyString(), anyString(), anyString(), anyMap());
    }
    ...
    void testParallelBranchCompletionAll() {
      ...
      verify(mockCommandBus).accept(eq("AfterJoin"), anyString(), eq("key"), anyString(), anyMap());
    }
      ...
      ProcessConfiguration config =
          new ProcessConfiguration() {
            ...
            public ProcessGraph defineProcess() {
              ...
              steps.put(
                  "ParallelStep",
                  new ProcessStep("ParallelStep", null, new ProcessStep.Terminal()));
    ...
    void testParallelBranchMissingInReply() {
      ...
      ProcessInstance instance =
          ProcessInstance.create(processId, "ParallelProcess", "key", "JoinStep", Map.of())
              .withStatus(ProcessStatus.RUNNING);
      ...
      CommandReply reply =
          CommandReply.completed(
              commandId, processId, Map.of("result", "success") // No parallelBranch field
              );
  ...
  /**
   * Parallel process configuration for testing ParallelStep -> [Branch1, Branch2, Branch3] ->
   * JoinStep -> AfterJoin
   */
  ...
  /**
   * Parallel process that ends after parallel execution ParallelStep -> [Branch1, Branch2, Branch3]
   * -> JoinStep (end)
   */
  ...
  private static class TestProcessGraph extends ProcessGraph {
    ...
    TestProcessGraph(
        ...
        boolean requiresCompensationOverride,
        boolean isParallelOverride) {
