...
class AggregatesTest {
    ...
    void testSnapshotVariousKeys() {
        assertThat(Aggregates.snapshot("order-456"))
            .contains("order-456");

        assertThat(Aggregates.snapshot("payment:abc:xyz"))
            .contains("payment:abc:xyz");

        assertThat(Aggregates.snapshot("simple"))
            .contains("simple");
    }
    ...
    void testSnapshotValidJson() {
        ...
        assertThatNoException().isThrownBy(() ->
            Jsons.fromJson(result, java.util.Map.class)
        );
    }
}
