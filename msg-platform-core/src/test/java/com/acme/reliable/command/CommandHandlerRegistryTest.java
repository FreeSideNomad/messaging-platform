...
class CommandHandlerRegistryTest {
    ...
    class HandlerRegistrationTests {
        ...
        void testRegisterHandler() {
            Function<CommandMessage, CommandReply> handler = cmd ->
                CommandReply.completed(cmd.commandId(), cmd.correlationId(), Map.of());
        ...
        void testRegisterDuplicateHandler() {
            Function<CommandMessage, CommandReply> handler1 = cmd ->
                CommandReply.completed(cmd.commandId(), cmd.correlationId(), Map.of());
            Function<CommandMessage, CommandReply> handler2 = cmd ->
                CommandReply.completed(cmd.commandId(), cmd.correlationId(), Map.of());
        ...
        void testRegisterMultipleHandlers() {
            ...
            CommandMessage cmd1 = new CommandMessage(UUID.randomUUID(), UUID.randomUUID(), "CreateUser", "{}");
            CommandMessage cmd2 = new CommandMessage(UUID.randomUUID(), UUID.randomUUID(), "DeleteUser", "{}");
    ...
    class CommandHandlingTests {
        ...
        void testHandleInvokesHandler() {
            ...
            CommandMessage command = new CommandMessage(commandId, correlationId, "CreateUser", "{\"name\":\"John\"}");
        ...
        void testHandleHandlerThrowsException() {
            ...
            Function<CommandMessage, CommandReply> faultyHandler = cmd -> {
    ...
    class IntegrationTests {
        ...
        void testMultipleCommandsSequentially() {
            Function<CommandMessage, CommandReply> createHandler = cmd ->
                CommandReply.completed(cmd.commandId(), cmd.correlationId(), Map.of("action", "created"));
            Function<CommandMessage, CommandReply> updateHandler = cmd ->
                CommandReply.completed(cmd.commandId(), cmd.correlationId(), Map.of("action", "updated"));

            registry.registerHandler("CreateUser", createHandler);
            registry.registerHandler("UpdateUser", updateHandler);

            CommandMessage createCmd = new CommandMessage(UUID.randomUUID(), UUID.randomUUID(), "CreateUser", "{}");
            ...
            CommandMessage updateCmd = new CommandMessage(UUID.randomUUID(), UUID.randomUUID(), "UpdateUser", "{}");
        ...
        void testHandlerState() {
            int[] counter = {0};
            Function<CommandMessage, CommandReply> statefulHandler = cmd -> {
                counter[0]++;
                return CommandReply.completed(cmd.commandId(), cmd.correlationId(), Map.of("count", counter[0]));
            };

            registry.registerHandler("CountCommand", statefulHandler);

            CommandMessage cmd1 = new CommandMessage(UUID.randomUUID(), UUID.randomUUID(), "CountCommand", "{}");
            CommandReply reply1 = registry.handle(cmd1);
            assertThat(reply1.data()).containsEntry("count", 1);

            CommandMessage cmd2 = new CommandMessage(UUID.randomUUID(), UUID.randomUUID(), "CountCommand", "{}");
        ...
        void testComplexPayload() {
            Function<CommandMessage, CommandReply> handler = cmd -> {
                ...
                return CommandReply.completed(cmd.commandId(), cmd.correlationId(), Map.of("processed", true));
            };
