...
class CommandMessageTest {
  ...
  void testCommandMessageWithNullCorrelation() {
    ...
    CommandMessage message = new CommandMessage(commandId, null, "CreateUser", "{}");
  ...
  void testCommandMessageInequality() {
    ...
    CommandMessage message1 =
        new CommandMessage(UUID.randomUUID(), correlationId, "CreateUser", "{}");
  ...
  void testCommandMessageSerialization() {
    ...
    assertThat(json).contains("\"commandType\":\"CreateUser\"").contains("\"payload\":");
  }
  ...
  void testCommandMessageEmptyPayload() {
    CommandMessage message =
        new CommandMessage(UUID.randomUUID(), UUID.randomUUID(), "NoDataCommand", "");
  ...
  void testCommandMessageComplexPayload() {
    String complexPayload =
        "{\"user\":{\"name\":\"John\",\"age\":30},\"metadata\":{\"source\":\"api\"}}";

    CommandMessage message =
        new CommandMessage(UUID.randomUUID(), UUID.randomUUID(), "CreateUser", complexPayload);
