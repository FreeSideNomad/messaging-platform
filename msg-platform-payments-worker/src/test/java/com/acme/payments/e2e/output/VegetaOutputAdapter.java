...
/**
 * Adapter for generating Vegeta HTTP load test target files.
 * Uses inline JSON format (no external file references).
 */
@Slf4j
public class VegetaOutputAdapter {
    ...
    public void writeSequencedTargets(E2ETestScenario scenario, String outputDir) throws IOException {
        ...
        log.info("Wrote {} opening credit targets to {}", scenario.getOpeningTransactions().size(), creditsFile);
        ...
        log.info("Wrote {} funding transaction targets to {}", scenario.getFundingTransactions().size(), fundingFile);
        ...
        log.info("Vegeta target generation complete. Total commands: {}", scenario.getMetrics().totalCommands());
    }

    /**
     * Write account creation process targets
     */
    public void writeAccountTargets(List<InitiateCreateAccountProcess> commands, Path outputFile) throws IOException {
     ...
     */
    public void writeTransactionTargets(List<CreateTransactionCommand> commands, Path outputFile) throws IOException {
        try (PrintWriter writer = new PrintWriter(Files.newBufferedWriter(outputFile))) {
     ...
     */
    public void writePaymentTargets(List<InitiateSimplePaymentCommand> commands, Path outputFile) throws IOException {
        try (PrintWriter writer = new PrintWriter(Files.newBufferedWriter(outputFile))) {
     ...
     */
    private void writeHttpTarget(PrintWriter writer, String method, String path, Object body) throws IOException {
