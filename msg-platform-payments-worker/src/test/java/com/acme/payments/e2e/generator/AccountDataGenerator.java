 ...
 *
 * <p>Key features: - Skewed balance distribution (90% of accounts < 100K) - Automatic limit
 * calculation based on balance - Support for multiple currencies - Configurable limit-based
 * percentage
 *
 * <p>Uses InitiateCreateAccountProcess which orchestrates the full account creation including
 * limits setup.
 */
public class AccountDataGenerator extends BaseDataGenerator {
  ...
  public InitiateCreateAccountProcess generateAccount(int limitBasedPercentage) {
    ...
    Map<PeriodType, Money> limits =
        limitBased ? calculateLimits(initialBalance, currencyCode) : null;

    return new InitiateCreateAccountProcess(
        customerId, currencyCode, transitNumber, accountType, limitBased, limits);
  }
  ...
  /** Select an account type with weighted distribution: 70% CHECKING, 20% SAVINGS, 10% others */
