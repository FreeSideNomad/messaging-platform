...
/**
 * Orchestrates generation of complete E2E test scenarios.
 * Maintains referential integrity across accounts, transactions, and payments.
 */
@Slf4j
public class E2ETestScenarioBuilder {
    ...
    public E2ETestScenario build(TestScenarioConfig config) {
        log.info("Building E2E test scenario with config: {} accounts, {}-{} payments, {}-{} funding",
            config.accountCount(), config.minPaymentsPerAccount(), config.maxPaymentsPerAccount(),
            config.minFundingPerAccount(), config.maxFundingPerAccount());
        ...
        for (int i = 0; i < config.accountCount(); i++) {
            ...
            // Generate account
            InitiateCreateAccountProcess accountCommand = accountGenerator.generateAccount(config.limitBasedAccountPercentage());
            ...
            CreateTransactionCommand openingTxn = transactionGenerator.generateOpeningCredit(
                metadata.accountId(),
                openingAmount
            );
            ...
            List<CreateTransactionCommand> funding = transactionGenerator.generateFundingTransactions(
                metadata.accountId(),
            ...
            // Generate payments
            List<InitiateSimplePaymentCommand> payments = paymentGenerator.generatePayments(
                metadata.customerId(),
                metadata.accountId(),
                metadata.currencyCode(),
                metadata.limits(),
        ...
        log.info("Generated {} accounts, {} opening txns, {} funding txns, {} payments",
            accountCommands.size(), openingTransactions.size(),
            fundingTransactions.size(), paymentCommands.size());
    ...
    private AccountMetadata createAccountMetadata(InitiateCreateAccountProcess command) {
        ...
        if (command.limitBased() && command.limits() != null) {
            ...
            initialBalance = hourlyLimit.amount()
