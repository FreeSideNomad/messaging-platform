...
/**
 * Complete E2E test scenario containing all generated commands
 * and metadata for validation and output generation.
 */
@Getter
public class E2ETestScenario {
    ...
    private TestScenarioMetrics calculateMetrics() {
        int totalCommands = accountCommands.size() + openingTransactions.size() +
                           fundingTransactions.size() + paymentCommands.size();

        long fxPaymentCount = paymentCommands.stream()
