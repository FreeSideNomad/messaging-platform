...
/**
 * Base generator providing common utilities for E2E test data generation. All other generators
 * should extend this class to access Faker and utility methods.
 */
@Getter
public abstract class BaseDataGenerator {
  ...
  /**
   * Generate a skewed amount using inverse transform sampling. This creates a distribution where a
   * certain percentile is at the threshold.
   *
   * <p>For example, if skewPercentile=0.9 and threshold=100000, then 90% of values will be below
   * 100000.
   *
   ...
   */
  protected BigDecimal generateSkewedAmount(
      BigDecimal min, BigDecimal max, double skewPercentile, BigDecimal threshold) {
    ...
    double alpha =
        Math.log(skewPercentile)
            / Math.log(
                (threshold.doubleValue() - min.doubleValue())
  ...
  protected BigDecimal calculatePercentage(
      BigDecimal amount, double percentage, BigDecimal minimum) {
    BigDecimal calculated =
        amount.multiply(BigDecimal.valueOf(percentage / 100.0)).setScale(2, RoundingMode.HALF_UP);
