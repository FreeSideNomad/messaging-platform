...
/**
 * Command to create all limits for an account in one batch operation. This command is typically
 * executed as part of the CreateAccount process when limitBased=true.
 */
public record CreateLimitsCommand(
        UUID accountId, String currencyCode, Map<PeriodType, Money> limits) implements DomainCommand {
        ...
        limits.forEach(
                (periodType, money) -> {
                    ...
                    if (!money.currencyCode().equals(currencyCode)) {
                        throw new IllegalArgumentException(
                                "All limit amounts must match account currency: "
                                        + currencyCode
                                        + " (found "
                                        + money.currencyCode()
                                        + " for "
