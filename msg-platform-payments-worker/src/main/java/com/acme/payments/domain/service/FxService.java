...
public class FxService {
    ...
    public FxContract bookFx(BookFxCommand cmd) {
        log.info("Booking FX: {} -> {} for customer {}",
            cmd.debitAmount(), cmd.creditAmount(), cmd.customerId());

        // Calculate FX rate
        BigDecimal rate = cmd.creditAmount().amount()
            .divide(cmd.debitAmount().amount(), 6, RoundingMode.HALF_UP);
        ...
        log.info("FX contract created: {} at rate {}",
            fxContract.getFxContractId(), rate);
    ...
    public void unwindFx(UUID fxContractId, String reason) {
        ...
        FxContract fxContract = fxContractRepository.findById(fxContractId)
    ...
    /**
     * Command handler for UnwindFxCommand
