...
/**
 * Initializes required IBM MQ queues when the connection pool is created. If queues don't exist and
 * cannot be created, the application will exit. This runs before JMS listeners are registered to
 * ensure queues exist.
 */
@Singleton
...
public class MqQueueInitializer implements BeanCreatedEventListener<JMSConnectionPool> {
  ...
  public MqQueueInitializer(
      @io.micronaut.context.annotation.Value(
              "${mq.required-queues:APP.CMD.CreateUser.Q,APP.CMD.REPLY.Q}")
          String requiredQueuesConfig) {
