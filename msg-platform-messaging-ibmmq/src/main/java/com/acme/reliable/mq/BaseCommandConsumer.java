 ...
 * Provides generic command processing and reply handling logic that can be reused across bounded contexts.
 */
public abstract class BaseCommandConsumer {
    ...
    protected BaseCommandConsumer(CommandHandlerRegistry registry, CommandQueue commandQueue, String replyQueue) {
     ...
     * Extracts command metadata from JMS headers, delegates to CommandHandlerRegistry,
     * and sends reply back to the reply queue.
     */
    protected void processCommand(String commandType, String body, jakarta.jms.Message m) throws JMSException {
        ...
        try {
            ...
            log.info("Command processed: type={}, id={}, status={}",
                commandType, commandId, reply.status());
            ...
            // Send error reply
            CommandReply errorReply = CommandReply.failed(
                UUID.randomUUID(),
                correlationId,
                e.getMessage()
            );
    ...
    private void sendReply(UUID correlationId, CommandReply reply) {
        try {
            ...
            log.debug("Sent reply to queue: correlationId={}, status={}",
                correlationId, reply.status());
        } catch (Exception e) {
