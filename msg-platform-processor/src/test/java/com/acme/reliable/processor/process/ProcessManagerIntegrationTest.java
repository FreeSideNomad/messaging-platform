...
class ProcessManagerIntegrationTest implements TestPropertyProvider {
    ...
    void testCompleteProcess_WithDataMerging() {
        ...
        CommandReply reply1 = CommandReply.completed(
            commandId1,
            processId,
            Map.of("step1Result", "completed", "amount", 200.0)
        );
        processManager.handleReply(processId, commandId1, reply1);

        // Simulate Step2 completion
        UUID commandId2 = UUID.randomUUID();
        CommandReply reply2 = CommandReply.completed(
            commandId2,
            processId,
            Map.of("step2Result", "done", "finalAmount", 250.0)
        );
        ...
        assertTrue(log.size() >= 5); // ProcessStarted, StepStarted, StepCompleted (x2), ProcessCompleted
    }
    ...
    void testProcessFailure_WithRetry() {
        ...
        UUID commandId1 = UUID.randomUUID();
        CommandReply reply1 = CommandReply.failed(
            commandId1,
            processId,
    ...
    void testFindByStatus() {
        ...
        UUID processId1 = processManager.startProcess(
            "IntegrationTestProcess",
            "test-key-4",
            Map.of("test", "status1")
        );

        UUID processId2 = processManager.startProcess(
            "IntegrationTestProcess",
            "test-key-5",
            Map.of("test", "status2")
        );
    ...
    void testFindByBusinessKey() {
        ...
        UUID processId = processManager.startProcess(
            "IntegrationTestProcess",
            businessKey,
            Map.of("test", "findByKey")
        );

        // When
        Optional<ProcessInstance> found = processRepository.findByBusinessKey(
            "IntegrationTestProcess",
            businessKey
        );
