...
/**
 * Test configuration that provides no-op implementations for messaging infrastructure. This
 * replaces the real MQ/Kafka implementations in test environments.
 */
@Factory
@Requires(env = "test")
public class TestCommandQueueConfig {
  ...
  /**
   * No-op implementation of CommandQueue for testing. Just logs the send operation without actually
   * sending to MQ.
   */
  static class NoOpCommandQueue implements CommandQueue {
    ...
    public void send(String queue, String body, Map<String, String> headers) {
      LOG.debug(
          "Test CommandQueue.send() called - queue: {}, body length: {}, headers: {}",
          queue,
  ...
  /**
   * No-op implementation of EventPublisher for testing. Just logs the publish operation without
   * actually publishing to Kafka.
   */
  static class NoOpEventPublisher implements EventPublisher {
    ...
    public void publish(String topic, String key, String value, Map<String, String> headers) {
      LOG.debug(
          "Test EventPublisher.publish() called - topic: {}, key: {}, value length: {}, headers: {}",
          topic,
          key,
  ...
  /** No-op implementation of HandlerRegistry for testing. Returns a default JSON response. */
  static class NoOpHandlerRegistry implements HandlerRegistry {
    ...
    public String invoke(String commandName, String payload) {
      LOG.debug(
          "Test HandlerRegistry.invoke() called - commandName: {}, payload length: {}",
          commandName,
