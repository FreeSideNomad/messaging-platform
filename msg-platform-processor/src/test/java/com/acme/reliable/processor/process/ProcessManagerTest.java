...
class ProcessManagerTest {
    ...
    void testHandleReply_StepCompleted_MovesToNextStep() {
        ...
        ProcessInstance runningInstance = new ProcessInstance(
            processId,
            "TestProcess",
            "test-123",
        ...
        // Then
        // Verify instance was updated with completed event (twice: once for completion, once for executing next step)
        ArgumentCaptor<ProcessInstance> instanceCaptor = ArgumentCaptor.forClass(ProcessInstance.class);
        ArgumentCaptor<ProcessEvent> eventCaptor = ArgumentCaptor.forClass(ProcessEvent.class);
    ...
    void testHandleReply_StepFailed_RetriesWithBackoff() throws InterruptedException {
        ...
        assertTrue(duration >= 1000, "Expected backoff delay of at least 1 second, but was " + duration + "ms");

        // Verify step was retried
        verify(mockCommandBus, times(1)).accept(
            eq("Step1"),
            eq(processId + ":Step1"),
            any(),
            any(),
    ...
    void testHandleReply_TimedOut_FailsPermanently() {
        ...
        CommandReply reply = CommandReply.timedOut(
            commandId,
            processId,
            "Command exceeded maximum wait time"
        );
