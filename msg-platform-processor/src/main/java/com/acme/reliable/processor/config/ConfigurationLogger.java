...
/**
 * Logs effective configuration on application startup for visibility and troubleshooting. Disabled
 * in test environment to avoid configuration errors.
 */
@Singleton
...
public class ConfigurationLogger implements ApplicationEventListener<ServerStartupEvent> {
  ...
  public void onApplicationEvent(ServerStartupEvent event) {
    ...
    LOG.info(
        "  Worker Threads:     {} (Netty I/O worker threads for handling requests)", workerThreads);
    LOG.info(
        "  Parent Threads:     {} (Netty boss threads for accepting connections)", parentThreads);
    LOG.info("");
    ...
    LOG.info(
        "  Command Lease:      {} (How long a command is locked for processing)",
        timeoutConfig.getCommandLease());
    LOG.info(
        "  Max Backoff:        {} (Maximum retry backoff for outbox publishing)",
        timeoutConfig.getMaxBackoff());
    LOG.info(
        "  Sync Wait:          {} (HTTP response wait time; 0s = fully async)",
        timeoutConfig.getSyncWait());
    LOG.info(
        "  Async Mode:         {} (Returns HTTP 202 immediately if true)", timeoutConfig.isAsync());
    LOG.info(
        "  Outbox Sweep:       {} (How often to check for unpublished outbox entries)",
        timeoutConfig.getOutboxSweepInterval());
    LOG.info(
        "  Outbox Batch Size:  {} (Number of outbox entries processed per sweep)",
        timeoutConfig.getOutboxBatchSize());
    LOG.info(
        "  Claim Timeout:      {} (Timeout to reclaim stuck CLAIMED outbox messages)",
        timeoutConfig.getOutboxClaimTimeout());
    LOG.info("");

    // Messaging Configuration
    LOG.info("━━━ Messaging Configuration ━━━");
    LOG.info(
        "  JMS Consumers:      {} (IBM MQ message listeners enabled)",
        jmsConsumersEnabled ? "ENABLED" : "DISABLED");
    LOG.info(
        "  Command Prefix:     {} (IBM MQ command queue prefix)",
        messagingConfig.getQueueNaming().getCommandPrefix());
    LOG.info(
        "  Queue Suffix:       {} (IBM MQ queue name suffix)",
        messagingConfig.getQueueNaming().getQueueSuffix());
    LOG.info(
        "  Reply Queue:        {} (IBM MQ reply queue name)",
        messagingConfig.getQueueNaming().getReplyQueue());
    LOG.info(
        "  Event Prefix:       {} (Kafka topic prefix for events)",
        messagingConfig.getTopicNaming().getEventPrefix());
